...
def 同居男闺蜜(End_Chapter, End_Page):
...
def windows():
    ...
    def print_End_Chapter():
        ...
        tkinter.messagebox.showinfo(title='同居男闺蜜', message="你即将要从第"+str(End_Chapter)+"章开始爬取")

    def print_End_Page():
        global End_Page, End_Chapter
        End_Page = End_Page_2.get()
        tkinter.messagebox.showinfo(title='Hi', message="你即将要从第" + str(End_Chapter) + "章"+ str(End_Page) +"页开始爬取")
    ...
    # 窗口无法实时更新数据
    # End_Page_1 = tkinter.Label(window, text="请输入你要从第"+str(End_Chapter)+"章的第多少页开始爬取：", bg="blue", font=("Arial", 12), width=30, height=2)
    End_Page_1 = tkinter.Label(window, text="请输入你要从第多少页开始爬取：", bg="blue", font=("Arial", 12),width=30, height=2)
    End_Page_1.pack()
...
if __name__ == '__main__':
    windows()
    End_Chapter, End_Page = eval(End_Chapter), eval(End_Page)
    while True:
